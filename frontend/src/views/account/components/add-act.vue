<template>
  <dialog-panel title="添加账户" confirm-text="添加" :visible="visible" @cancel="cancel" @confirm="add" :confirm-loading="loading" width="500px">
  ...
  methods: {
    ...
    add() {
      this.$refs.accountForm.validate((v) => {
        if (v) {
          ...
          accountCreate(this.accountForm)
            .then((res) => {
    ...
    remoteMethod(query) {
      if (query.trim() !== "") {
        ...
        parentAccounts({ account_name: query })
          .then((res) => {
