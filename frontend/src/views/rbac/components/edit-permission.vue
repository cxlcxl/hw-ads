<template>
  <dialog-panel title="权限修改" confirm-text="保存" :visible="visible" @cancel="cancel" @confirm="save" :confirm-loading="loading" width="450px">
  ...
  methods: {
    ...
    save() {
      this.$refs.permission_update.validate((v) => {
        if (v) {
          this.loading = true
          permissionUpdate(this.permissionForm)
            .then((res) => {
