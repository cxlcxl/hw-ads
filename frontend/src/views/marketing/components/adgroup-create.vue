    ...
    <el-form :model="adgroupForm" ref="adgroupForm" label-width="120px" size="mini" :rules="adgroupRules">
      ...
      <el-col :span="24" style="border-top: 1px solid #e3e3e3;">
        ...
        <el-tabs v-model="editableTabsValue" type="card" closable @tab-remove="removeTab">
          <el-tab-pane v-for="(__adgroup, idx) in adgroupForm.adgroups" :label="__adgroup.adgroup_name" :name="__adgroup.tab_name"
            :key="__adgroup.tab_name">
            <el-form-item label="定向" prop="targeting_package_id" :prop="'adgroups.' + idx + '.targeting_package_id'"
              :rules="{required: true, message: '请选择定向'}">
            ...
            <el-form-item label="版位" prop="creative_size_id" :prop="'adgroups.' + idx + '.creative_size_id'"
              ...
              <adgroup-creative-size :creative-size-list="positionList[__adgroup.tab_name]" :ref="'creativeSize' + __adgroup.tab_name"
                :creative-size-id="__adgroup.creative_size_id" v-show="__adgroup.category !== ''" @handle-change="handleSelectPosition" />
            </el-form-item>

            <el-form-item label="投放日期" :prop="'adgroups.' + idx + '.adgroup_begin_date'" :rules="{required: true, message: '请选择投放开始日期'}">
            ...
            <el-form-item label="出价" :prop="'adgroups.' + idx + '.price'" :rules="{required: true, message: '请填写出价金额'}">
              ...
              <div v-else v-loading="loadings.priceLoading">
                <el-radio-group v-model="__adgroup.price_type" @change="handleChangePriceType($event, __adgroup.creative_size_id, idx)">
                  <el-radio-button :label="key" v-for="(label,key) in resources.pricing" v-if="pricing[__adgroup.tab_name].includes(label)">
            ...
            <el-form-item label="任务名称" :prop="'adgroups.' + idx + '.adgroup_name'" :rules="{required: true, message: '请填写任务名称'}">
...
export default {
  ...
  methods: {
    ...
    getCampaignInfo() {
      return new Promise((resolve, reject) => {
        campaignInfo(this.adgroupForm.campaign_id)
          .then((res) => {
    ...
    getPositionList(category, tabName) {
      ...
      positionList({ category, account_id: this.campaign.account_id, product_type: this.campaign.product_type })
        .then((res) => {
    ...
    getResources() {
      ...
      if (this.$route.query.hasOwnProperty("campaign_id") && this.$route.query.campaign_id !== "") {
        ...
        Promise.all([this.getPricing(), this.getPositionCategory(), this.getCampaignInfo()])
          .then(() => {
            targetingList({ campaign_id: this.adgroupForm.campaign_id, account_id: this.campaign.account_id })
              .then((res) => {
    ...
    handleChangePriceType(v, creative_size_id, idx) {
      ...
      this.minPrice = 0
      this.$set(this.adgroupForm.adgroups[idx], "price", 0)
      positionPrice({ creative_size_id, price_type: v })
        .then((res) => {
    ...
    refreshTracking() {
      this.loadings.priceLoading = true
      trackingRefresh({ app_id: this.campaign.app_id })
        .then((res) => {
          this.trackings = res.data
          this.loadings.priceLoading = false
        })
